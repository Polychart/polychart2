...
class PositionScale extends Scale
  ...
  _makeNone: () ->
    ...
    @f = @_NaNCheckWrap (value) =>
      if _.isObject(value)
        width = (m) -> (@range.max-@range.min-2*space) / m
        switch value.f
          when 'identity' then return value.v
          ...
          when 'lower'
            if not value.m then return @range.min + space
            else return (@range.min+space) + value.n*width(value.m)
      else
  ...
  _numWrapper: (domain, y) => @_NaNCheckWrap (value) =>
    ...
    if _.isObject(value)
      if value.t is 'scalefn'
        switch value.f
          when 'identity' then return value.v
          when 'middle'   then return y(value.v+domain.bw/2) #for log scale
          when 'max'      then return @range.max + value.v
          when 'min'      then return @range.min + value.v
          when 'upper', 'lower'
...
class Area extends Scale
  _makeNum: () => #range = [0, 1]
    ...
    sqrt = Math.sqrt
    ylin = poly.linear sqrt(@domain.min), min, sqrt(@domain.max), 10
    @f   = @_identityWrapper (x) -> ylin sqrt(x)
  _makeDate: () => @_makeNum()
...
class Palette extends Scale
  _makeCat: () => #TEMPORARY
    ...
    if n <= 9
      colors = [ "#E41A1C" # Vivid Red
               , "#377EB8" # Moderate Blue
               , "#4DAF4A" # Dark moderate lime green
               , "#984EA3" # Dark moderate magenta
               , "#FF7F00" # Pure (or mostly pure) orange
               , "#FFFF33" # Vivid yellow
               , "#A65628" # Dark orange [Brown tone]
               , "#F781BF" # Soft pink
               , "#999999" # Dark gray
               ]
      @f = (value) =>
...
poly.scale.make = (spec) ->
  if spec.type of poly.scale.classes
