...
_.keys(this.ticks));d=b.deleted;f=b.kept;b=b.added;console.log(d,f,b);n={};t=0;for(r=d.length;t<r;t++)q=d[t],this._delete(c,this.pts[q]);d=0;for(t=f.length;d<t;d++)q=f[d],n[q]=this._modify(c,this.pts[q],this.ticks[q],a);f=0;for(d=b.length;f<d;f++)q=b[f],n[q]=this._add(c,this.ticks[q],a);return this.pts=n};x.prototype.remove=function(a){var c,b,d;d=this.pts;for(c in d)b=d[c],this._delete(a,b);a.remove(this.title);this.title=null;return this.pts={}};x.prototype._add=function(a,c,b){var d;d={};d.tick=
a.add(this._makeTick(b,c));d.text=a.add(this._makeLabel(b,c));return d};x.prototype._delete=function(a,c){a.remove(c.tick);return a.remove(c.text)};x.prototype._modify=function(a,c,b,d){var f;f=[];f.tick=a.animate(c.tick,this._makeTick(d,b));f.text=a.animate(c.text,this._makeLabel(d,b));return f};x.prototype._makeLabel=function(c,b){return{type:"text",x:a.identity(c.right+15),y:a.identity(c.top+(15+12*b.index)+1),text:b.value,"text-anchor":"start"}};x.prototype._makeTick=function(c,b){var d,f,n,q;
f={type:"circle",x:a.identity(c.right+7),y:a.identity(c.top+(15+12*b.index)),color:a.identity("steelblue")};q=this.mapping;for(d in q)n=q[d],0<=y.call(g["const"].noLegend,d)||(n=n[0],f[d]=0<=y.call(this.aes,d)?b.location:null!=n.type&&"const"===n.type?a.identity(n.value):_.isObject(n)?a.identity(g["const"].defaults[d]):a.identity(n));0<=y.call(this.aes,"size")||(f.size=a.identity(5));return f};x.prototype._makeTitle=function(c,b){return{type:"text",x:a.identity(c.right+5),y:a.identity(c.top),text:b,
"text-anchor":"start"}};x.prototype.getDimension=function(){return{position:"right",height:this.height,width:15+this.maxwidth}};g.guide={};g.guide.axis=function(a){if("x"===a)return new m;if("y"===a)return new f;if("r"===a)return new j;if("t"===a)return new b};g.guide.legend=function(a){return new x(a)};this.poly=g}).call(this);
...
c[f]?0:a[f]>=c[f]?1*e:-1*e});d&&(b=b.slice(0,+(d-1)+1||9E9));a=_.uniq(_.pluck(b,a));return{meta:{levels:a,sorted:!0},filter:{"in":a}}};a=function(a,c,d,g){var e,p,k,l,j,s,t,r,A,F,G;k=_.clone(c);null==d&&(d={});c=function(a,c){var b;return d[a]=_.extend(null!=(b=d[a])?b:{},c)};if(a.trans){G=a.trans;l=0;for(A=G.length;l<A;l++){t=G[l];j=t.key;s=y(j,t,d[j]);e=s.trans;s=s.meta;r=0;for(F=k.length;r<F;r++)p=k[r],e(p);c(t.name,s)}}a.filter&&(k=_.filter(k,f(a.filter)));if(a.meta){e={};t=a.meta;for(j in t)s=
t[j],l=b(j,s,k),s=l.meta,l=l.filter,e[j]=l,c(j,s);k=_.filter(k,f(e))}if(a.stats&&a.stats.stats&&0<a.stats.stats.length){k=m(k,a.stats);s=a.stats.stats;a=0;for(j=s.length;a<j;a++)e=s[a],e=e.name,c(e,{type:"num"})}return g(k,d)};j=function(){return console.log("backendProcess")};c.data.frontendProcess=a;this.poly=c}).call(this);
